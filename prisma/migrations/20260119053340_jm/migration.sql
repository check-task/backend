...
CREATE TABLE `TaskPirority` (
    ...
    UNIQUE INDEX `TaskPirority_user_id_task_id_key`(`user_id`, `task_id`),
    PRIMARY KEY (`priority_id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- AddForeignKey
ALTER TABLE `TaskPirority` ADD CONSTRAINT `TaskPirority_user_id_fkey` FOREIGN KEY (`user_id`) REFERENCES `User`(`user_id`) ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `TaskPirority` ADD CONSTRAINT `TaskPirority_task_id_fkey` FOREIGN KEY (`task_id`) REFERENCES `Task`(`task_id`) ON DELETE CASCADE ON UPDATE CASCADE;
